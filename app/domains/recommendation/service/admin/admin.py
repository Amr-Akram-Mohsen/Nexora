...
def get_recommendation_stats():
    ...
    linked_items = db.session.execute(select(func.count(func.distinct(content_products.c.product_id)))).scalar() or 0
    total_impressions = db.session.execute(select(func.count()).select_from(RecommendationImpression)).scalar() or 0
    total_clicks = db.session.execute(select(func.count()).select_from(RecommendationClick)).scalar() or 0
...
def fetch_admin_matches_page(page, per_page, search, entity_type=None, ctr_range=None):
    ...
    if entity_type:
        base_stmt = base_stmt.where(select(RecommendationImpression.id).where(RecommendationImpression.context_id == context_id_expr).where(RecommendationImpression.entity_type == entity_type).exists())
    if ctr_range:
    ...
    if not content_ids:
        return (total, pages, [])
    detail_stmt = select(Content, Product).select_from(Content).join(content_products, Content.id == content_products.c.content_id).join(Product, Product.id == content_products.c.product_id).where(Content.id.in_(content_ids))
...
def get_admin_recommendation_health():
    ...
    if last_impression:
        ...
        if hours_since > 48:
            signals.append({'level': 'critical', 'message': f'No impressions recorded in the last {int(hours_since)} hours.'})
    else:
        signals.append({'level': 'critical', 'message': 'No impressions ever recorded.'})
    seven_days_ago = now - timedelta(days=7)
    imp_7d = db.session.scalar(select(func.count(RecommendationImpression.id)).where(RecommendationImpression.created_at >= seven_days_ago)) or 0
    click_7d = db.session.scalar(select(func.count(RecommendationClick.id)).where(RecommendationClick.created_at >= seven_days_ago)) or 0
    if imp_7d > 100:
        ctr_7d = click_7d / imp_7d * 100
        if ctr_7d < 0.5:
            signals.append({'level': 'warning', 'message': f'7-day overall CTR is dangerously low ({ctr_7d:.2f}%).'})
    if not signals:
        signals.append({'level': 'success', 'message': f'Recommendation system is healthy. {imp_7d} impressions in last 7 days.'})
        status = 'success'
    else:
...
def get_admin_recommendation_trend():
    ...
    click_stmt = select(func.date(RecommendationClick.created_at).label('day'), func.count(RecommendationClick.id).label('count')).where(RecommendationClick.created_at >= thirty_days_ago).group_by('day')
