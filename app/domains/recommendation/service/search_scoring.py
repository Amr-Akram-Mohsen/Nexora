...
CONTENT_INTENT_TERMS = {'how', 'guide', 'tutorial', 'review', 'tips', 'what', 'why', 'news'}
SHOPPING_INTENT_TERMS = {'buy', 'best', 'price', 'compare', 'top', 'vs', 'deal', 'deals', 'product', 'products'}
PRODUCT_HINT_TERMS = {'phone', 'laptop', 'camera', 'headphones', 'watch', 'perfume', 'tablet'}
TEXT_WEIGHT = 4.0
...
def _text_relevance(query, result):
    ...
    secondary = ' '.join([str(result.get('description') or ''), str(_value(target, 'preview_text', '')), str(_value(target, 'description', '')), str(_value(category, 'name', '')), str(_value(brand, 'name', '')), _joined_names(result.get('topics')), _joined_names(result.get('brands'))]).lower()
    score = 0.0
