...
class UserInterest(db.Model):
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("target_type IN ('product', 'article')", name="ck_target_type"),
    )
...
class UserEntityInterest(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint("(entity_id IS NOT NULL) OR (category_id IS NOT NULL)", name="ck_entity_reference"),
        db.Index("ix_user_entity_interest_ref", "entity_id", "category_id"),
    )
