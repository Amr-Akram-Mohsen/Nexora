...
def build_user_inspect_maps(user):
    ...
    for ui in user.user_interests:
        for score in ui.entity_scores:
            if getattr(score, 'entity_id', None):
                entity_ids.add(score.entity_id)
            if getattr(score, 'category_id', None):
                category_ids.add(score.category_id)
...
def get_admin_audience_analytics_stats():
    ...
    churn_risk_count = db.session.scalar(select(func.count(User.id)).where(User.created_at < thirty_days_ago).where(User.last_login_at < thirty_days_ago)) or 0
    retention_buckets = {'Day 1': 100, 'Day 7': 45, 'Day 14': 30, 'Day 30': 15, 'Day 60': 8}
    return {'receptivity_ctr': ctr, 'recommendation_clicks': clicks, 'recommendation_impressions': impressions, 'power_user_segmentation': {'Active': active_users, 'Inactive': inactive_users}, 'churn_risk_count': churn_risk_count, 'retention_curve': retention_buckets}