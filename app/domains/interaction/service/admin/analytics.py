...
def get_interactions_breakdown() -> dict:
    ...
    rows = db.session.execute(select(func.sum(case((View.id != None, 1), else_=0)).label('views')).select_from(View))
...
def get_analytics_dashboard_data() -> dict:
    ...
    trends = {'comments': _get_trend_data(Comment, Comment.created_at, thirty_days_ago), 'reactions': _get_trend_data(Reaction, Reaction.created_at, thirty_days_ago), 'views': _get_trend_data(View, View.created_at, thirty_days_ago), 'saves': _get_trend_data(Save, Save.created_at, thirty_days_ago), 'shares': _get_trend_data(Share, Share.created_at, thirty_days_ago), 'clicks': _get_trend_data(ProductClick, ProductClick.created_at, thirty_days_ago)}
    deltas = {'comments': _get_delta_data(Comment, Comment.created_at, fourteen_days_ago, seven_days_ago), 'reactions': _get_delta_data(Reaction, Reaction.created_at, fourteen_days_ago, seven_days_ago), 'views': _get_delta_data(View, View.created_at, fourteen_days_ago, seven_days_ago), 'saves': _get_delta_data(Save, Save.created_at, fourteen_days_ago, seven_days_ago), 'shares': _get_delta_data(Share, Share.created_at, fourteen_days_ago, seven_days_ago), 'clicks': _get_delta_data(ProductClick, ProductClick.created_at, fourteen_days_ago, seven_days_ago)}
    sentiment_rows = db.session.execute(select(Comment.sentiment, func.count(Comment.id)).where(Comment.sentiment.isnot(None)).group_by(Comment.sentiment)).all()
    sentiment_dist = {r[0]: r[1] for r in sentiment_rows}
    spam_stmt = select(cast(Comment.created_at, Date).label('date'), func.count().label('count')).where(Comment.created_at >= thirty_days_ago, Comment.sentiment == 'spam').group_by(cast(Comment.created_at, Date))
    ...
    top_saves_stmt = select(Save.target_type, Save.target_id, func.count(Save.id).label('cnt')).group_by(Save.target_type, Save.target_id).order_by(desc('cnt')).limit(5)
    ...
    for r in top_saves_rows:
        ...
        if r.target_type == 'content':
            ...
            if i:
                target_name = i.name
        top_saves.append({'target_type': r.target_type, 'target_id': r.target_id, 'name': target_name, 'count': r.cnt})
    country_rows = db.session.execute(select(ProductClick.country, func.count(ProductClick.id)).where(ProductClick.country.isnot(None)).group_by(ProductClick.country).order_by(desc(func.count(ProductClick.id))).limit(10)).all()
...
def get_hourly_engagement_heatmap() -> list:
    ...
    for hour in range(24):
        total = views_hc.get(hour, 0) + comments_hc.get(hour, 0) + reactions_hc.get(hour, 0) + saves_hc.get(hour, 0) + shares_hc.get(hour, 0)
