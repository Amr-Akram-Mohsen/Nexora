...
def get_admin_clicks_page(page, per_page, target, destination):
    stmt = (
        select(
            ProductStoreLink.id.label("link_id"),
            ProductStoreLink.affiliate_url,
            Store.name.label("store_name"),
            Product.id.label("product_id"),
            Product.name.label("item_name"),
            func.count(ProductClick.id).label("click_count"),
            func.max(ProductClick.created_at).label("latest_click")
        )
    ...
    stmt = stmt.group_by(
        ...
        Product.id,
        Product.name
    ).order_by(func.max(ProductClick.created_at).desc())
