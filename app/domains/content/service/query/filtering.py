...
from app.domains.taxonomy.models import Category, Entity, Brand
...
def get_contents_render(filter_by_columns: tuple=('section',), filter_values: tuple=(None,), rows_count=None, exclude_ids=None, session=None):
    ...
    video_filter = and_(Content.object_type == 'video', exists(select(Video.id).join(VideoComment, VideoComment.video_id == Video.id).where(Video.id == Content.object_id)))
    stmt = build_content_stmt(active_only=True, published_only=True, eager_load='list', extra_filters=[or_(article_filter, video_filter)])
    if exclude_ids:
...
def _get_paginated_contents(filters=None, allowed_filters=None, section_id=None, active_only=True, published_only=True, sort_by=None, sort_dir=None, page=1, per_page=20, session=None, for_admin=False, exclude_ids=None):
    ...
    if for_admin and sort_by:
        _CONTENT_SORT_MAP = {'id': Content.id, 'published_at': Content.published_at, 'ingested_at': Content.ingested_at, 'view_count': Content.view_count, 'like_count': Content.like_count, 'comment_count': Content.comment_count, 'share_count': Content.share_count, 'save_count': Content.save_count, 'score': Content.score, 'title': Content.title}
        sort_col = _CONTENT_SORT_MAP.get(sort_by, Content.published_at)
        match sort_dir and sort_dir.lower():
            case 'asc':
                stmt = stmt.order_by(sort_col.asc())
            case _:
                stmt = stmt.order_by(sort_col.desc())
    else:
        match filters.get('sort'):
            case 'oldest':
...
def get_candidate_contents_for_item(product, session=None):
    ...
    if product.brand_id:
        ...
        if brand:
            ...
            if entity:
                stmt = stmt.join(ContentEntity, (ContentEntity.content_id == Content.id) & (ContentEntity.entity_id == entity.id))
