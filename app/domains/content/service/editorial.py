...
def assess_video_description(description: str | None) -> dict:
    ...
    if any((phrase in description.lower() for phrase in sponsor_phrases)):
        ...
        high_count += 1
    timestamp_pattern = re.compile('\\d+:\\d+')
...
def assess_article_extraction(target) -> dict:
    ...
    if is_scraped and word_count >= 300 and (not any((w['severity'] == 'high' for w in warnings))):
        ...
        extraction_health = 'thin'
    return {'has_content_text': has_content_text, 'has_content_html': has_content_html, 'has_description': has_description, 'word_count': word_count, 'read_time_minutes': read_time_minutes, 'quality_score': quality_score, 'is_scraped': is_scraped, 'extraction_status': extraction_status, 'has_images': has_images, 'image_count': image_count, 'has_extended_metadata': has_extended_metadata, 'metadata_key_count': metadata_key_count, 'warnings': warnings, 'extraction_health': extraction_health, 'extended_metadata': extended_metadata}