...
def get_admin_taxonomy_analytics():
    ...
    total_entities = 0
    orphans = 0
    entities_config = [(Category, ~db.session.query(Content.id).filter(Content.category_id == Category.id).exists(), ~db.session.query(Product.id).filter(Product.category_id == Category.id).exists()), (Brand, ~db.session.query(Product.id).filter(Product.brand_id == Brand.id).exists(), None), (Entity, ~db.session.query(ContentEntity.content_id).filter(ContentEntity.entity_id == Entity.id).exists(), None), (Section, ~db.session.query(Content.id).filter(Content.section_id == Section.id).exists(), None)]
...
def get_admin_taxonomy_related_metadata(entity, entity_type):
    ...
    if entity_type == 'category':
        ...
        variants_count = db.session.scalar(select(func.count(ProductVariant.id)).join(Product, Product.id == ProductVariant.product_id).where(Product.category_id == entity.id)) or 0
        data['avg variants per product'] = str(round(variants_count / total_items, 1) if total_items > 0 else 0)
        items_with_images = db.session.scalar(select(func.count(func.distinct(ProductImage.product_id))).join(Product, Product.id == ProductImage.product_id).where(Product.category_id == entity.id)) or 0
        data['image coverage'] = f'{round(items_with_images / total_items * 100)}%' if total_items > 0 else '0%'
        items_with_content = db.session.scalar(select(func.count(func.distinct(content_products.c.product_id))).join(Product, Product.id == content_products.c.product_id).where(Product.category_id == entity.id)) or 0
        ...
        data['product count'] = str(total_items)
        avg_price = db.session.scalar(select(func.avg(ProductVariant.price)).join(Product, Product.id == ProductVariant.product_id).where(Product.brand_id == entity.id))
        data['average price'] = f'${avg_price:.2f}' if avg_price else '—'
        items_with_images = db.session.scalar(select(func.count(func.distinct(ProductImage.product_id))).join(Product, Product.id == ProductImage.product_id).where(Product.brand_id == entity.id)) or 0
        data['image coverage'] = f'{round(items_with_images / total_items * 100)}%' if total_items > 0 else '0%'
        top_items = db.session.execute(select(Product).where(Product.brand_id == entity.id).order_by(Product.click_count.desc()).limit(5)).scalars().all()
        ...
        _format_breakdown_and_engagement(type_breakdown, engagement)
        rel_cats = db.session.scalar(select(func.count(func.distinct(Content.category_id))).join(ContentEntity, ContentEntity.content_id == Content.id).filter(ContentEntity.entity_id == entity.id)) or 0
        data['related categories'] = str(rel_cats)
        rel_brands = db.session.scalar(select(func.count(func.distinct(ContentEntity.entity_id))).join(Entity, Entity.id == ContentEntity.entity_id).filter(ContentEntity.content_id.in_(select(ContentEntity.content_id).where(ContentEntity.entity_id == entity.id))).filter(Entity.entity_type == 'brand')) or 0
        ...
        _format_breakdown_and_engagement(type_breakdown, engagement)
        cat_count = db.session.scalar(select(func.count(func.distinct(Content.category_id))).filter(Content.section_id == entity.id)) or 0
        data['category count'] = str(cat_count)
        rel_brands = db.session.scalar(select(func.count(func.distinct(ContentEntity.entity_id))).join(Content, Content.id == ContentEntity.content_id).join(Entity, Entity.id == ContentEntity.entity_id).filter(Content.section_id == entity.id).filter(Entity.entity_type == 'brand')) or 0
        data['related brands'] = str(rel_brands)
        ...
        field_mapping = {'gender_facet': Content.gender_id, 'intent_facet': Content.intent_id, 'price_tier_facet': Content.price_tier_id}
...
def get_admin_source_inspect_raw(id):
    ...
    analytics = db.session.query(func.avg(Article.quality_score), func.avg(Article.word_count), func.count(Article.id).filter(Article.is_content_scraped == True), func.min(Content.published_at), func.max(Content.published_at)).select_from(Content).join(Article, Content.object_id == Article.id).filter(Content.source_id == id, Content.object_type == 'article').first()
    type_counts = db.session.execute(select(Content.object_type, func.count(Content.id)).where(Content.source_id == id).group_by(Content.object_type)).all()
    channel_counts = db.session.execute(select(Content.ingestion_origin, func.count(Content.id)).where(Content.source_id == id).where(Content.ingestion_origin.is_not(None)).group_by(Content.ingestion_origin)).all()
    category_counts = db.session.scalar(select(func.count(func.distinct(Content.category_id))).where(Content.source_id == id)) or 0
    status_counts = db.session.execute(select(Article.status, func.count(Article.id)).join(Content, Content.object_id == Article.id).where(Content.source_id == id).where(Content.object_type == 'article').group_by(Article.status)).all()
    eng_stats = db.session.execute(select(func.sum(Content.view_count).label('views'), func.sum(Content.like_count).label('likes'), func.sum(Content.save_count).label('saves'), func.sum(Content.comment_count).label('comments')).where(Content.source_id == id)).first()
    fetch_health = db.session.execute(select(func.sum(LastAPIFetch.success_count).label('success'), func.sum(LastAPIFetch.failure_count).label('failures'), func.max(LastAPIFetch.consecutive_failures).label('consecutive'), func.max(LastAPIFetch.last_fetched_at).label('last_fetch')).where(func.lower(LastAPIFetch.source) == source.slug.lower())).first()
    primary_count = db.session.scalar(select(func.count(Article.id)).where(Article.primary_source_id != None).join(ArticleSource, Article.primary_source_id == ArticleSource.id).where(ArticleSource.source_id == id)) or 0
...
def get_facet_rows_data(model, field_id_name, page, per_page, search, health):
    ...
    for a in pagination.items:
        ...
        if c_count == 0:
            h_status = 'unused'
        data = {'id': a.id, 'name': a.name, 'content count': str(c_count), 'health': h_status}
        serialized.append(data)
    return (serialized, pagination)