...
def get_search_items(query_str, limit=80):
    ...
    stmt = select(Product).options(*get_item_card_load_options()).where(Product.search_vector.op('@@')(search_query), Product.ingestion_status.in_(['published', 'ready'])).order_by(title_boost, rank.desc(), Product.review_count.desc(), Product.view_count.desc(), Product.created_at.desc())
    if limit:
