...
def get_item_load_options(profile='detail'):
    ...
    minimal = [db.joinedload(Product.brand), db.joinedload(Product.category)]
    card = [*minimal, db.selectinload(Product.images), db.selectinload(Product.variants).selectinload(ProductVariant.store_links).selectinload(ProductStoreLink.store)]
