...
def build_item_search_vector(product):
    return (
        func.setweight(func.to_tsvector('english', func.coalesce(product.name, '')), 'A')
        .op('||')(func.setweight(func.to_tsvector('english', func.coalesce(product.description, '')), 'B'))
        .op('||')(func.setweight(func.to_tsvector('english', func.coalesce(product.search_text, '')), 'C'))
    )
...
def populate_item_search_fields(product):
    ...
    if hasattr(product, 'attributes') and product.attributes:
        attribute_names = ' '.join((attr.name for attr in product.attributes if attr.name))
    spec_text = ''
    ...
    product.search_text = ' '.join(
        filter(None, [product.name, product.description, brand_name, category_name, attribute_names, spec_text, product.product_type])
    )
...
def get_search_items(query_str, limit=80):
    ...
    rank = func.ts_rank_cd(Product.search_vector, search_query)
    title_boost = sa_case((func.lower(Product.name).startswith(query_str.lower()), 0), else_=1)
    stmt = (
        select(Product)
        .options(*get_item_card_load_options())
        .where(Product.search_vector.op('@@')(search_query), Product.ingestion_status.in_(['published', 'ready']))
        .order_by(title_boost, rank.desc(), Product.review_count.desc(), Product.view_count.desc(), Product.created_at.desc())
    )
    if limit:
...
def get_filtered_items(active_filters, page=1, per_page=24):
    ...
    if active_filters.get('category'):
        stmt = stmt.join(Product.category).where(Category.slug.in_(active_filters['category']))
    ...
    if has_store_filter or has_price_filter or is_price_sort:
        if is_price_sort:
            ...
            if has_store_filter:
                stmt = stmt.where(ProductVariant.store_links.any(ProductStoreLink.store.has(Store.slug.in_(active_filters['store']))))
            ...
            variant_conds = []
            if has_store_filter:
                variant_conds.append(ProductVariant.store_links.any(ProductStoreLink.store.has(Store.slug.in_(active_filters['store']))))
            if min_p is not None:
