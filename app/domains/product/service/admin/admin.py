...
def get_admin_item_health_stats():
    ...
    link_stats = db.session.execute(select(func.count(func.distinct(ProductVariant.product_id)).label('items_with_links'), func.count(func.distinct(case((ProductStoreLink.last_synced_at >= seven_days_ago, ProductVariant.product_id), else_=None))).label('recent_sync')).select_from(ProductVariant).join(ProductStoreLink, ProductStoreLink.variant_id == ProductVariant.id).where(ProductStoreLink.is_active.is_(True))).first()
    ...
    type_dist_rows = db.session.execute(select(Product.product_type, func.count(Product.id)).group_by(Product.product_type).order_by(func.count(Product.id).desc())).all()
    product_type_distribution = [{'type': r[0] or 'Uncategorized', 'count': r[1]} for r in type_dist_rows]
    top_items_rows = db.session.execute(select(Product.id, Product.name, Product.click_count, Product.view_count, Product.save_count, Product.like_count).where(Product.view_count > 10).order_by((Product.click_count * 1.0 / Product.view_count).desc()).limit(5)).all()
...
def load_admin_item_aggregates(page_ids):
    ...
    for r in min_price_rows:
        if r.product_id not in min_price_map:
            min_price_map[r.product_id] = {'price': float(r.min_price), 'max_price': float(r.max_price) if r.max_price else None, 'currency': r.currency}
    store_rows = db.session.execute(select(ProductVariant.product_id, func.count(ProductStoreLink.id).label('active_links'), func.max(ProductStoreLink.last_synced_at).label('last_synced_at'), func.max(ProductStoreLink.old_price).label('max_old_price')).join(ProductStoreLink, ProductStoreLink.variant_id == ProductVariant.id).where(ProductVariant.product_id.in_(page_ids), ProductStoreLink.is_active.is_(True)).group_by(ProductVariant.product_id)).all()
    store_info_map = {r.product_id: {'active_links': int(r.active_links), 'last_synced_at': r.last_synced_at, 'has_discount': r.max_old_price is not None and float(r.max_old_price) > 0} for r in store_rows}
    image_rows = db.session.execute(select(ProductImage.product_id, func.count(ProductImage.id).label('image_count')).where(ProductImage.product_id.in_(page_ids)).group_by(ProductImage.product_id)).all()
...
def build_admin_items_query(args, sort_col, sort_dir):
    ...
    if availability and availability != 'all':
        stmt = stmt.where(Product.variants.any(ProductVariant.store_links.any((ProductStoreLink.is_active == True) & (ProductStoreLink.availability == availability))))
...
def get_admin_store_inspect_raw(id):
    ...
    if not store:
        return None
    product_count = db.session.scalar(select(func.count(func.distinct(ProductVariant.product_id))).join(ProductStoreLink, ProductStoreLink.variant_id == ProductVariant.id).where(ProductStoreLink.store_id == id)) or 0
    now = datetime.now(timezone.utc)
    stale_date = now - timedelta(days=7)
    stats = db.session.execute(select(func.count(ProductStoreLink.id).label('total_links'), func.sum(case((ProductStoreLink.is_active == True, 1), else_=0)).label('active_links'), func.sum(case((ProductStoreLink.is_active == False, 1), else_=0)).label('inactive_links'), func.sum(case((ProductStoreLink.last_synced_at == None, 1), else_=0)).label('never_synced'), func.sum(case((ProductStoreLink.last_synced_at < stale_date, 1), else_=0)).label('stale_links'), func.sum(case((ProductStoreLink.availability == 'OutOfStock', 1), else_=0)).label('out_of_stock'), func.max(ProductStoreLink.last_synced_at).label('last_synced'), func.count(func.distinct(ProductStoreLink.program_name)).label('program_count'), func.avg(ProductStoreLink.commission_rate).label('avg_commission'), func.max(ProductStoreLink.commission_rate).label('max_commission'), func.sum(case((ProductStoreLink.commission_rate != None, 1), else_=0)).label('with_commission'), func.sum(case((ProductStoreLink.commission_rate == None, 1), else_=0)).label('without_commission'), func.sum(case((ProductStoreLink.tracking_code != None, 1), else_=0)).label('with_tracking'), func.min(ProductStoreLink.price).label('min_price'), func.avg(ProductStoreLink.price).label('avg_price'), func.max(ProductStoreLink.price).label('max_price'), func.sum(case(((ProductStoreLink.old_price != None) & (ProductStoreLink.old_price > ProductStoreLink.price), 1), else_=0)).label('with_discount'), func.avg(case(((ProductStoreLink.old_price != None) & (ProductStoreLink.old_price > ProductStoreLink.price), (ProductStoreLink.old_price - ProductStoreLink.price) / ProductStoreLink.old_price * 100), else_=None)).label('avg_discount_pct'), func.sum(case((ProductStoreLink.price == None, 1), else_=0)).label('null_price')).where(ProductStoreLink.store_id == id)).first()
    currency_mix_rows = db.session.execute(select(ProductStoreLink.currency, func.count(ProductStoreLink.id)).where(ProductStoreLink.store_id == id).where(ProductStoreLink.currency != None).group_by(ProductStoreLink.currency)).all()
    currency_mix_list = [{'label': c[0], 'detail': c[1]} for c in currency_mix_rows] if currency_mix_rows else []
    all_syncs = db.session.execute(select(ProductStoreLink.last_synced_at).where(ProductStoreLink.store_id == id).where(ProductStoreLink.last_synced_at != None)).all()
    total_days = 0
    for dt, in all_syncs:
