...
def get_admin_stores_page(page, per_page, search, network, country, sync_staleness):
    ...
    if search:
        term = f'%{search}%'
        stmt = stmt.where(or_(Store.name.ilike(term), Store.slug.ilike(term), Store.website.ilike(term)))
    if network:
    ...
    if page_store_ids:
        product_count_rows = db.session.execute(select(ProductStoreLink.store_id, func.count(func.distinct(ProductVariant.product_id)).label('product_count')).join(ProductVariant, ProductVariant.id == ProductStoreLink.variant_id).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
        product_count_map = {r.store_id: r.product_count for r in product_count_rows}
        clicks_rows = db.session.execute(select(ProductStoreLink.store_id, func.count(ProductClick.id).label('clicks')).join(ProductClick, ProductClick.product_store_link_id == ProductStoreLink.id).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
        clicks_map = {r.store_id: r.clicks for r in clicks_rows}
        views_rows_q = db.session.execute(select(ProductStoreLink.store_id, func.count(func.distinct(View.id)).label('views')).join(ProductVariant, ProductVariant.id == ProductStoreLink.variant_id).join(View, (View.target_id == ProductVariant.product_id) & (View.target_type == 'product')).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
        views_map = {r.store_id: r.views for r in views_rows_q}
        latest_activity_rows = db.session.execute(select(ProductStoreLink.store_id, func.max(Product.created_at).label('latest_created_at')).join(ProductVariant, ProductVariant.id == ProductStoreLink.variant_id).join(Product, Product.id == ProductVariant.product_id).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
        latest_activity_map = {r.store_id: r.latest_created_at for r in latest_activity_rows}
        avg_comm_rows = db.session.execute(select(ProductStoreLink.store_id, func.avg(ProductStoreLink.commission_rate).label('avg_comm')).where(ProductStoreLink.store_id.in_(page_store_ids)).where(ProductStoreLink.commission_rate != None).group_by(ProductStoreLink.store_id)).all()
        avg_comm_map = {r.store_id: r.avg_comm for r in avg_comm_rows}
        sync_age_rows = db.session.execute(select(ProductStoreLink.store_id, func.max(ProductStoreLink.last_synced_at).label('last_synced')).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
        sync_age_map = {r.store_id: r.last_synced for r in sync_age_rows}
        oos_rows = db.session.execute(select(ProductStoreLink.store_id, func.sum(case((ProductStoreLink.availability == 'OutOfStock', 1), else_=0)).label('oos_count'), func.count(ProductStoreLink.id).label('total_links'), func.sum(case((ProductStoreLink.is_active == True, 1), else_=0)).label('active_links')).where(ProductStoreLink.store_id.in_(page_store_ids)).group_by(ProductStoreLink.store_id)).all()
...
def get_admin_store_health_stats():
    ...
    synced_today = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.last_synced_at >= today_start)) or 0
    synced_this_week = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.last_synced_at >= week_start)) or 0
    never_synced = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.last_synced_at.is_(None))) or 0
    out_of_stock = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.availability == 'OutOfStock')) or 0
    oos_stores_rows = db.session.execute(select(Store.name, func.count(ProductStoreLink.id)).join(ProductStoreLink, ProductStoreLink.store_id == Store.id).where(ProductStoreLink.availability == 'OutOfStock').group_by(Store.name).order_by(func.count(ProductStoreLink.id).desc()).limit(5)).all()
    oos_by_store = [{'name': r[0], 'count': r[1]} for r in oos_stores_rows]
    month_start = now - timedelta(days=30)
    checked_in_24h = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.last_checked_at >= today_start)) or 0
    deeplink_refreshed_30d = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.deeplink_generated_at >= month_start)) or 0
    never_checked = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.last_checked_at.is_(None))) or 0
    never_had_deeplink = db.session.scalar(select(func.count(ProductStoreLink.id)).where(ProductStoreLink.deeplink_generated_at.is_(None))) or 0
    avail_rows = db.session.execute(select(ProductStoreLink.availability, func.count(ProductStoreLink.id)).group_by(ProductStoreLink.availability)).all()
    ...
    recent_syncs = db.session.scalars(select(ProductStoreLink.last_synced_at).where(ProductStoreLink.last_synced_at >= month_start)).all()
    sync_cadence = _build_sync_cadence(recent_syncs, now)
    store_syncs = db.session.execute(select(Store.name, ProductStoreLink.last_synced_at).join(ProductStoreLink, ProductStoreLink.store_id == Store.id).where(ProductStoreLink.last_synced_at != None)).all()
...
def get_admin_store_affiliate_stats():
    ...
    program_rows = db.session.execute(select(ProductStoreLink.program_name, func.count(ProductStoreLink.id)).where(ProductStoreLink.program_name != None).group_by(ProductStoreLink.program_name).order_by(func.count(ProductStoreLink.id).desc()).limit(10)).all()
    ...
    commission_rows = db.session.execute(select(Store.name, func.avg(ProductStoreLink.commission_rate)).join(ProductStoreLink, ProductStoreLink.store_id == Store.id).where(ProductStoreLink.commission_rate != None).group_by(Store.name).order_by(func.avg(ProductStoreLink.commission_rate).desc()).limit(10)).all()
    ...
    tracking_coverage = {'With Tracking': with_tracking, 'Without Tracking': without_tracking}
...
def get_admin_store_pricing_stats():
    ...
    avg_discount_scalar = db.session.scalar(select(func.avg((ProductStoreLink.old_price - ProductStoreLink.price) / ProductStoreLink.old_price * 100.0)).where(ProductStoreLink.old_price != None).where(ProductStoreLink.price != None).where(ProductStoreLink.old_price > ProductStoreLink.price).where(ProductStoreLink.old_price > 0))
    avg_discount_percentage = round(float(avg_discount_scalar), 1) if avg_discount_scalar else 0.0
    currency_rows = db.session.execute(select(ProductVariant.currency, func.count(ProductStoreLink.id)).join(ProductVariant, ProductVariant.id == ProductStoreLink.variant_id).where(ProductVariant.currency != None).group_by(ProductVariant.currency)).all()
    currency_mix = {r[0] if r[0] else 'Unknown': r[1] for r in currency_rows}
    discount_rows = db.session.execute(select(Store.name, func.avg((ProductStoreLink.old_price - ProductStoreLink.price) / ProductStoreLink.old_price * 100.0)).join(ProductStoreLink, ProductStoreLink.store_id == Store.id).where(ProductStoreLink.old_price != None).where(ProductStoreLink.price != None).where(ProductStoreLink.old_price > ProductStoreLink.price).where(ProductStoreLink.old_price > 0).group_by(Store.name).order_by(func.avg((ProductStoreLink.old_price - ProductStoreLink.price) / ProductStoreLink.old_price * 100.0).desc()).limit(5)).all()
    discount_depth_ranking = [{'name': r[0], 'avg_discount': round(float(r[1]), 1)} for r in discount_rows]
    staleness_query = db.session.execute(select(Store.name, ProductStoreLink.last_synced_at).join(ProductStoreLink, ProductStoreLink.store_id == Store.id)).all()
    price_staleness_grid = _build_price_staleness_grid(staleness_query, stale_date)
    item_avail_query = db.session.execute(select(ProductVariant.product_id, ProductStoreLink.availability).join(ProductStoreLink, ProductStoreLink.variant_id == ProductVariant.id)).all()
    item_avail_map = {}
    ...
    store_price_query = db.session.execute(select(Store.name, ProductStoreLink.price).join(ProductStoreLink, ProductStoreLink.store_id == Store.id)).all()
    store_price_map = {}
    ...
    return {'null_price_count': null_price_count, 'stale_price_count': stale_price_count, 'avg_discount_percentage': avg_discount_percentage, 'currency_mix': currency_mix, 'discount_depth_ranking': discount_depth_ranking, 'price_staleness_grid': price_staleness_grid, 'all_oos_items_count': all_oos_items_count, 'high_null_price_stores': high_null_price_stores}