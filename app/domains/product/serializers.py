...
def serialize_item_detail(product):
    ...
    detailed_images = [{'id': img.id, 'image_url': img.image_url, 'position': img.position} for img in product.images]
...
def _calculate_item_completeness_score(product, price_info, store_info, has_image, has_specs):
    ...
    if product.description and len(product.description) > 10:
        completeness_points += 1
    if store_info.get('active_links', 0) > 0:
        completeness_points += 1
    if price_info.get('price') is not None:
        completeness_points += 1
    if has_specs:
        completeness_points += 1
    if product.searchable_attributes and len(product.searchable_attributes) > 0:
        completeness_points += 1
