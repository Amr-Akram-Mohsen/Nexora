...
class Product(db.Model):
    ...
    linked_contents = db.relationship('Content', secondary=content_products, back_populates='linked_products')
    variants = db.relationship('ProductVariant', back_populates='product', cascade='all, delete-orphan', lazy='selectin')
    images = db.relationship('ProductImage', back_populates='product', cascade='all, delete-orphan', order_by='ProductImage.position', lazy='selectin', overlaps='images')
    specifications = db.relationship('ProductSpecification', back_populates='product', cascade='all, delete-orphan', lazy='selectin')
    ...
    def structured_details(self):
        ...
        if self.product_type == 'technology':
            return {'highlights': self.pick_keys({**data.get('display', {}), **data.get('platform', {}), **data.get('battery', {})}, ['size', 'chipset', 'type']), 'quick_details': data, 'groups': data}
        elif self.product_type == 'perfumes':
            fp = data.get('fragrance_profile', {})
            return {'scent': {'top': fp.get('top_notes'), 'heart': fp.get('heart_notes'), 'base': fp.get('base_notes')}, 'meta': self.pick_keys(fp, ['concentration', 'scent_family']), 'performance': data.get('performance', {}), 'quick_details': data.get('performance', {}), 'groups': data}
