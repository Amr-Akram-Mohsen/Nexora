...
def sources_rows():
    ...
    if search:
        term = f"%{search}%"
        stmt = stmt.where(or_(Source.name.ilike(term), Source.slug.ilike(term), Source.domain.ilike(term)))
    stmt = stmt.order_by(Source.authority_score.desc(), Source.name.asc())
    pagination = db.paginate(stmt, page=page, per_page=per_page, error_out=False)
    page_source_ids = [s.id for s in pagination.items]
    ...
    if page_source_ids:
        content_agg = db.session.execute(
            select(Content.source_id,
                   func.count(Content.id).label("content_count"),
                   func.max(Content.ingested_at).label("latest_ingested_at"),
                   func.sum(Content.view_count + Content.like_count + Content.dislike_count + Content.save_count + Content.comment_count).label("engagement"))
            .where(Content.source_id.in_(page_source_ids))
            .group_by(Content.source_id)
        ).all()
        content_agg_map = {r.source_id: r for r in content_agg}
    ...
    for s in pagination.items:
        ...
        if not s.is_active:
            ...
            status_val = "healthy"
        serialized.append({"id": s.id, "name": s.name, "slug": s.slug, "domain": s.domain,
                           "is_active": s.is_active, "content_count": content_count,
                           "last_crawl": last_crawl, "success_rate": success_rate,
                           "failure_count": failure_count, "engagement": engagement, "status": status_val})
...
def stores_rows():
    ...
    if page_store_ids:
        product_count_rows = db.session.execute(
            select(ItemStoreLink.store_id, func.count(func.distinct(ItemVariant.item_id)).label("product_count"))
            .join(ItemVariant, ItemVariant.id == ItemStoreLink.variant_id)
            .where(ItemStoreLink.store_id.in_(page_store_ids))
            .group_by(ItemStoreLink.store_id)
        ).all()
        ...
        product_count_map, clicks_map, views_map = {}, {}, {}
    ...
    for st in pagination.items:
        ...
        if not st.is_active:
            ...
            status_val = "healthy"
        serialized.append({"id": st.id, "name": st.name, "slug": st.slug, "website": st.website,
                           "affiliate_network": st.affiliate_network, "is_active": st.is_active,
                           "product_count": product_count, "clicks": clicks,
                           "ctr": ctr, "conversions": 0, "status": status_val})
