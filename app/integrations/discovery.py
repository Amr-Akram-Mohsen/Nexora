...
from app.application.discovery.query_generator import (
...
class DiscoveryManager:
            ...
            for cat_group in categories_data:
                ...
                for child in cat_group.get("children", []):
                    ...
                    if allowed_sources is None:
                        allowed_sources = DEFAULT_SOURCE_ALIGNMENT.get(
                            sec_slug,
                            []
                        )
                    ...
                    if (
                        source_filter
                        and source_filter not in allowed_sources
                    ):
                        logger.debug(
    ...
    def get_section_slugs(self) -> List[str]:
        return [
            generate_slug(s["name"])
            for s in self.taxonomy.get("sections", [])
        ]