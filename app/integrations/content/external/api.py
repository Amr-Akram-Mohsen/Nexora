...
def should_refetch(section: str, query_text: str, hours: int = 24) -> bool:
    ...
    is_last_success = rec.success_count > 0 and (
        not rec.last_failed_at or rec.last_fetched_at > rec.last_failed_at
    )
    ...
    if rec.last_failed_at:
        ...
        if elapsed <= timedelta(minutes=FAILURE_RETRY_MINS):
            remaining_mins = (
                timedelta(minutes=FAILURE_RETRY_MINS) - elapsed
            ).total_seconds() / 60
            log_cooldown_skip(
                logger,
                "failure_cooldown",
...
def mark_fetched(
    ...
    if rec:
        ...
        if source and not rec.source:
            rec.source = source
        if normalized_query and not rec.normalized_query:
            rec.normalized_query = normalized_query
        if etag:
            rec.etag = etag
        if last_modified:
