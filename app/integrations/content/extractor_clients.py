...
def _extract_mercury(url: str, api_key: str) -> dict | None:
    try:
        ...
        content_text = data.get("title", "") # Mercury sometimes only returns HTML, text extraction might be needed
        
        if not content_html and not content_text:
            return None
            
        word_count = data.get("word_count", len(content_text.split()) if content_text else 0)
        quality = score_content_quality(content_html, content_text)
