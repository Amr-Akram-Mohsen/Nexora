...
def _collapse_internal_whitespace(soup: BeautifulSoup) -> None:
    ...
    for tag in soup.find_all(
        ["table", "thead", "tbody", "tr", "td", "th", "div", "ul", "ol", "li"]
    ):
    ...
    for img in soup.find_all("img"):
        ...
        for attr in lazy_candidates:
            ...
            if candidate.startswith("http") or candidate.startswith("//"):
                img["src"] = (
                    candidate if candidate.startswith("http") else "https:" + candidate
                )
                break
        ...
        img.attrs = {
            ...
            if k in ("src", "alt", "width", "height", "loading", "class")
        }
...
def enhance_article_html(html: str) -> str:
    ...
    for table in soup.find_all("table"):
        if table.parent and "article-full-text__table-wrapper" in (
            table.parent.get("class") or []
        ):
...
def _extract_via_trafilatura(html_source: str, url: str) -> str | None:
    ...
    try:
        ...
        html = (
            ...
            .replace("</main>", "</div>")
            .replace("<head>", "")
            .replace("</head>", "")  # strip any stray xml head
        )
        return html
    except Exception as e:
...
def scrape_article_content(url: str) -> str | None:
    ...
    if enhanced and enhanced.strip():
        ...
        log_scrape_success(
            logger, url, words=word_count, source="trafilatura_or_jsonld"
        )
        return enhanced
