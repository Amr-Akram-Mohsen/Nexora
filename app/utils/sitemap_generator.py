...
from app.models import Article, Item, Section, Category, Brand
...
def generate_static_sitemap(app):
    ...
    with app.test_request_context(base_url=base_url):
        ...
        for section in sections:
            pages.append([
                url_for('main.sections', section_slug=section.slug, _external=True),
                today
            ])
        ...
        for item in items:
            last_mod = (item.updated_at or item.created_at or datetime.now(timezone.utc)).date().isoformat()
            pages.append([
