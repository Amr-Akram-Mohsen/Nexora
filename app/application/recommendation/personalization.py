...
def get_personalized_feed_workflow(user_id, limit=12):
    """
    Analyzes the user's recent history and saved items to extract
    dominant taxonomy elements (categories/brands).
    Returns a personalized list of recommended items.
    """
    ...
    # Try fetching item recommendations
    ...
    if item_limit > 0:
        ...
        if category_slugs:
            cat_items = get_popular_items(
                category_slugs=tuple(category_slugs),
                limit=item_limit + len(seen_item_ids),
                session=db.session
            )
            item_recs.extend(cat_items)
            
        if brand_slugs and len(item_recs) < (item_limit + len(seen_item_ids)):
            brand_items = get_popular_items(
                brand_slugs=tuple(brand_slugs),
                limit=item_limit + len(seen_item_ids),
                session=db.session
            )
            existing_item_ids = {i["id"] for i in item_recs}
            for b in brand_items:
                if b["id"] not in existing_item_ids:
                    item_recs.append(b)
    ...
    # We shuffle or leave as is (content first, then items). Let's leave as is to group them or let grid handle it.
    return recommended
