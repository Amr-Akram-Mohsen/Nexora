...
def get_personalized_feed_workflow(user_id, limit=12):
    ...
    content_recs = []
    if category_slugs:
        cat_recs = get_popular_contents(category_slugs=tuple(category_slugs), limit=content_limit + len(seen_content_ids), session=db.session)
        content_recs.extend(cat_recs)
    if brand_slugs and len(content_recs) < content_limit + len(seen_content_ids):
        brand_recs = get_popular_contents(brand_slugs=tuple(brand_slugs), limit=content_limit + len(seen_content_ids), session=db.session)
    ...
    if item_limit > 0:
        item_recs = []
        if category_slugs:
            cat_items = get_popular_items(category_slugs=tuple(category_slugs), limit=item_limit + len(seen_product_ids), session=db.session)
            item_recs.extend(cat_items)
        if brand_slugs and len(item_recs) < item_limit + len(seen_product_ids):
            brand_items = get_popular_items(brand_slugs=tuple(brand_slugs), limit=item_limit + len(seen_product_ids), session=db.session)
