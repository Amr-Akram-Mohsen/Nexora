from app.domains.product.service import get_item_by_id, get_related_items, serialize_item
...
def get_item_page_data(product_id):
    """Product page orchestration: single DB load.

    Previously this module issued:
      1. get_item_by_id(serialize=True)   → ORM load + serialize
      2. get_item_by_id(serialize=False)  → second ORM load (same product!)
      3. db.session.query(Product.view_count) → third scalar query
    ...
    # Content articles/reviews that reference this product (Content↔Product matcher)
...
def record_item_view(product_id, user, ip_address):
    """Records an product view interaction."""
    record_view(
